...
card maps are existential. No lex leader, fixed deleted-vertex matching,
degree-class assumption, or prescribed local map is imposed. Every actual
parent isomorphism is blocked as an equality system (CEGAR).
"""
import argparse
import itertools
import json
...
def run(core, out, seconds=60, drop=0, preblock=False, resume=None, roots=2, certify=False):
                ...
                if not answer:
                    ...
                    if certify:
                        ...
                        if not proof or proof[-1].strip() != '0':
                            proof.append('0')
                        (out / 'unsat.drup').write_text('\n'.join(proof) + '\n')
                        report['status'] = 'UNSAT_PROOF_PENDING_INDEPENDENT_CHECK'
                        report['proof_lines'] = len(proof)
                    break
...
def main():
    ...
    args = ap.parse_args()
    if args.control:
        run((0, 0), args.out, args.seconds, drop=3 if args.control == 'partial' else 0, preblock=args.preblock, resume=args.resume, roots=args.roots, certify=args.certify)
    else:
        ...
        run(core, args.out, args.seconds, preblock=args.preblock, resume=args.resume, roots=args.roots, certify=args.certify)
