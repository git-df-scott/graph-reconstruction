...
def score(rows, n):
    if not connected(rows, n) or len({bin(r).count("1") for r in rows}) < 2:
        return -1
    s = sum(symmetric(rows, n, v) for v in range(n))
    if symmetric(rows, n):
        s -= n
    return s
        ...
        for _ in range(a.steps):
            ...
            if s >= cur:
                ...
                if cur == n:
                    ...
                    # keep walking on the plateau to collect neighbours
            else:
                rows[x] ^= 1 << y; rows[y] ^= 1 << x
                stale += 1
                if stale > 400:
                    break
        print(f"restart {r}: best {cur}, distinct optima so far {len(found)}", file=sys.stderr, flush=True)
